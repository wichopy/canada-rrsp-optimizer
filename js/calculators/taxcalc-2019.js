$(document).ready(function() {
  ...
  // AutoNumeric init
  AutoNumeric.multiple("#taxcalcfields input", {
    currencySymbol: "$ ",
    decimalPlaces: 0,
    maximumValue: "999999.99"
  });
  AutoNumeric.multiple("#taxcalcanswers .js-answer:not(.js-percent)", 0, {
    currencySymbol: "$ ",
    decimalPlaces: 0,
    minimumValue: "-999999.99"
  });
  AutoNumeric.multiple("#taxcalcanswers .js-answer.js-percent", 0, {
    currencySymbol: " %",
  ...
  if (window.lang && window.lang === "fr") {
    $("#taxcalcfields input, #taxcalcanswers .js-answer:not(.js-percent)").each(
      function() {
        AutoNumeric.getAutoNumericElement(this).update({
          digitGroupSeparator: " ",
          decimalCharacter: ",",
          currencySymbol: " $",
          currencySymbolPlacement: "s"
  ...
  function taxCalc(inputs) {
    ...
    this.can_emp_amt = Math.min(
      1222,
      this.income_emp + this.income_se
    ); /*2019*/
    ...
    this.emp_cpp_premiums =
      (Math.max(
        0,
        Math.min(this.income_emp, this.cpp_max) - this.cpp_exemption
      ) *
        this.cppqpp_rate) /
      2;
    ...
    if (this.income_se > 0 && this.income_emp == 0) {
      ...
      this.se_cpp_premiums =
        Math.max(
          0,
          Math.min(this.income_se, this.cpp_max) - this.cpp_exemption
        ) * this.cppqpp_rate;
    } else if (this.income_se > 0 && this.income_emp > 0) {
      this.se_cpp_premiums =
        Math.max(
          0,
          Math.min(this.total_income, this.cpp_max) - this.cpp_exemption
        ) * this.cppqpp_rate;
    ...
    this.ei_premiums =
      Math.max(0, Math.min(this.income_emp, this.ei_max)) * this.ei_rate;

    //Payroll Deductions
    this.payroll_deductions = this.emp_cpp_premiums + this.ei_premiums;

    //Payroll Tax Credits
    this.payroll_tax_credits =
      this.emp_cpp_premiums + this.se_cpp_premiums / 2 + this.ei_premiums;
    ...
    this.f_tax = Math.max(
      this.f_tax - (12069 + this.can_emp_amt + this.payroll_tax_credits) * 0.15,
      0
    ); /*2019*/
    this.f_tax = Math.max(
      this.f_tax -
        this.taxable_e_div * 0.150198 -
        this.taxable_i_div * 0.090301,
      0
    ); /*2019*/
    ...
    switch (this.prov) {
      ...
      case "AB":
        ...
        this.p_tax = Math.max(
          this.p_tax - (19369 + this.payroll_tax_credits) * 0.1,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.1 - this.taxable_i_div * 0.0204,
          0
        ); /*2019*/
        break;
      ...
      // British Columbia
      case "BC":
        if (this.income <= 40707) {
          /*2019*/
          this.p_tax = this.income * 0.0506;
          this.m_rate = this.m_rate + 0.0506;
        } else if (this.income <= 81416) {
          /*2019*/
          this.p_tax = (this.income - 40707) * 0.077 + 2060;
          this.m_rate = this.m_rate + 0.077;
        } else if (this.income <= 93476) {
          /*2019*/
          this.p_tax = (this.income - 81416) * 0.105 + 5194;
          this.m_rate = this.m_rate + 0.105;
        } else if (this.income <= 113506) {
          /*2019*/
          this.p_tax = (this.income - 93476) * 0.1229 + 6461;
          this.m_rate = this.m_rate + 0.1229;
        } else if (this.income <= 153900) {
          /*2019*/
          this.p_tax = (this.income - 113506) * 0.147 + 8922;
          this.m_rate = this.m_rate + 0.147;
        } else {
          /*2019*/
          this.p_tax = (this.income - 153900) * 0.168 + 14860;
          this.m_rate = this.m_rate + 0.168;
        }
        this.p_tax = Math.max(
          this.p_tax - (10682 + this.payroll_tax_credits) * 0.0506,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.12 - this.taxable_i_div * 0.0196,
          0
        ); /*2019*/
        break;
      ...
      case "MB":
        ...
        this.p_tax = Math.max(
          this.p_tax - (9626 + this.payroll_tax_credits) * 0.108,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax -
            this.taxable_e_div * 0.08 -
            this.taxable_i_div * 0.007835,
          0
        ); /*2019*/
        break;
      ...
      case "NB":
        ...
        this.p_tax = Math.max(
          this.p_tax - (10264 + this.payroll_tax_credits) * 0.0968,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.14 - this.taxable_i_div * 0.0275,
          0
        ); /*2019*/
        break;
      ...
      case "NL":
        ...
        this.p_tax = Math.max(
          this.p_tax - (9414 + this.payroll_tax_credits) * 0.087,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.054 - this.taxable_i_div * 0.035,
          0
        ); /*2019*/
        break;
      ...
      case "NS":
        ...
        this.p_tax = Math.max(
          this.p_tax - (this.ns_bpa + this.payroll_tax_credits) * 0.0879,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax -
            this.taxable_e_div * 0.0885 -
            this.taxable_i_div * 0.0299,
          0
        ); /*2019*/
        break;
      ...
      case "NT":
        if (this.income <= 43137) {
          /*2019*/
          this.p_tax = this.income * 0.059;
          this.m_rate = this.m_rate + 0.059;
        } else if (this.income <= 86277) {
          /*2019*/
          this.p_tax = (this.income - 43137) * 0.086 + 2545;
          this.m_rate = this.m_rate + 0.086;
        } else if (this.income <= 140267) {
          /*2019*/
          this.p_tax = (this.income - 86277) * 0.122 + 6255;
          this.m_rate = this.m_rate + 0.122;
        } else {
          /*2019*/
        ...
        this.p_tax = Math.max(
          this.p_tax - (14811 + this.payroll_tax_credits) * 0.059,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.115 - this.taxable_i_div * 0.06,
          0
        ); /*2019*/
        break;
      ...
      case "NU":
        ...
        this.p_tax = Math.max(
          this.p_tax - (13618 + this.payroll_tax_credits) * 0.04,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax -
            this.taxable_e_div * 0.0551 -
            this.taxable_i_div * 0.0261,
          0
        ); /*2019*/
        break;
      ...
      case "ON":
        ...
        this.p_tax = Math.max(
          this.p_tax - (10582 + this.payroll_tax_credits) * 0.0505,
          0
        ); /*2019*/
        ...
        } else if (this.income > 91101) {
          /*2019*/ /* higher surtax starts here */
          this.m_rate = this.m_rate + 0.1116 * 0.56;
        } else if (this.income > 87813) {
          /*2019*/ /* next tax bracket starts here */
          this.m_rate = this.m_rate + 0.1116 * 0.2;
        } else if (this.income > 77313) {
          /*2019*/ /* lower surtax starts here */
        ...
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.1 - this.taxable_i_div * 0.032863,
          0
        ); /*2019*/
      ...
      case "PE":
        ...
        this.p_tax = Math.max(
          this.p_tax - (9160 + this.payroll_tax_credits) * 0.098,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.105 - this.taxable_i_div * 0.0274,
          0
        ); /*2019*/
      ...
      // Quebec
      case "QC":
        if (this.income <= 43790) {
          /*2019*/
          this.p_tax = this.income * 0.15;
          this.m_rate = this.m_rate + 0.15;
        } else if (this.income <= 87575) {
          /*2019*/
          this.p_tax = (this.income - 43790) * 0.2 + 6568.5;
          this.m_rate = this.m_rate + 0.2;
        } else if (this.income <= 106555) {
          /*2019*/
        ...
        this.p_tax = Math.max(
          this.p_tax -
            this.taxable_e_div * 0.1178 -
            this.taxable_i_div * 0.0555,
          0
        ); /*2019*/
        break;
      ...
      // Saskatchewan
      case "SK":
        if (this.income <= 45225) {
          /*2019*/
          this.p_tax = this.income * 0.105;
          this.m_rate = this.m_rate + 0.105;
        } else if (this.income <= 129214) {
          /*2019*/
          this.p_tax = (this.income - 45225) * 0.125 + 4749;
          this.m_rate = this.m_rate + 0.125;
        } else {
          /*2019*/
          this.p_tax = (this.income - 129214) * 0.145 + 15247;
          this.m_rate = this.m_rate + 0.145;
        }
        this.p_tax = Math.max(
          this.p_tax - (16065 + this.payroll_tax_credits) * 0.105,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.11 - this.taxable_i_div * 0.03362,
          0
        ); /*2019*/
        break;
      ...
      case "YT":
        ...
        this.p_tax = Math.max(
          this.p_tax -
            (12069 + this.can_emp_amt + this.payroll_tax_credits) * 0.064,
          0
        ); /*2019*/
        this.p_tax = Math.max(
          this.p_tax - this.taxable_e_div * 0.1202 - this.taxable_i_div * 0.023,
          0
        ); /*2019*/
        break;
    }
    ...
    // Total Tax
    this.tax =
      this.f_tax + this.p_tax + this.payroll_deductions + this.se_cpp_premiums;

    // Effective Rate
    this.e_rate =
      this.tax /
        (this.income_a + this.income_se + this.c_g + this.e_div + this.i_div) ||
      0;

    // Tax Refund or Owing
    // doesn't include payroll deductions
    this.refund =
      this.tax_paid - (this.f_tax + this.p_tax + this.se_cpp_premiums);
  }
  ...
  function refreshPage() {
    ...
    if (!inputs.prov) {
      // don't run with no province set
      return;
    }
    ...
    var $showCPPEIPremiums = $taxCalcAnswers
      .find(".showCPPEIPremiums")
      .addClass("hidden");
    var $showSelfEmpCPP = $taxCalcAnswers
      .find(".showSelfEmpCPP")
      .addClass("hidden");
    ...
    AutoNumeric.set(
      ".answerTotalInc",
      calc.income_a + calc.income_se + calc.c_g + calc.e_div + calc.i_div
    );
    AutoNumeric.set(
      ".answerAfterTax",
      calc.income_a +
